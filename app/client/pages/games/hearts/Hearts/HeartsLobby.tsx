...
const HeartsLobby: React.FC = () => {
  const { lobby, createRoom, enterRoom } = useLobby(EGame.HEARTS, DEFAULT_GAME_OPTIONS);

  if (!lobby) {
    return null;
  }

  return <Lobby game={EGame.HEARTS} rooms={lobby.rooms} onEnterRoom={enterRoom} onCreateRoom={createRoom} />;
};
